# Claude Code Project Instructions — base64-kotlin

## Project Overview

This is **base64-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`base64`](https://crates.io/crates/base64).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/base64/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/base64>
